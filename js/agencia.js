  ...
  function renderCards(paquetes) {
    ...
    if (paquetes.length === 0) {
      grid.innerHTML =
        ...
        '<p style="font-size:16px">Próximamente nuevos paquetes. ¡Consultanos!</p></div>';
      return;
    }

    var html = "";
    paquetes.forEach(function(pkg, i) {

      // Imagen: si tiene foto subida la mostramos, sino gradiente de color
      var imgStyle = pkg.imagenUrl
        ? "background-image:url('" + pkg.imagenUrl + "');background-size:cover;background-position:center;"
        : "background:" + GRADIENTS[i % GRADIENTS.length] + ";";

      // Botones de descarga (solo si existen las URLs)
      var btnPdf = pkg.pdfUrl
        ? '<a href="' + pkg.pdfUrl + '" target="_blank" class="ag-card__file-btn">📄 Itinerario</a>'
        : "";

      var btnFlyer = pkg.flyerUrl
        ? '<a href="' + pkg.flyerUrl + '" target="_blank" class="ag-card__file-btn">🗺️ Flyer</a>'
        : "";

      var archivos = (btnPdf || btnFlyer)
        ? '<div class="ag-card__files">' + btnPdf + btnFlyer + '</div>'
        : "";

      // Link de WhatsApp con el nombre del paquete precompletado
      var mensaje = "Hola, quiero consultar sobre el paquete: " + pkg.nombre;
        ...
        '<article class="ag-card" data-category="' + pkg.categoria + '">' +
          ...
          '<div class="ag-card__body">' +
            ...
            '<div class="ag-card__footer">' +
              ...
              '<a href="' + wspUrl + '" target="_blank" rel="noopener" class="ag-card__cta">Consultar este viaje</a>' +
            '</div>' +
          '</div>' +
        '</article>';
    });
    ...
    // Animacion de entrada y filtro activo
    applyEntranceAnim();
    var activeTab = document.querySelector(".ag-tab.is-active");
